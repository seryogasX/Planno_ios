...
public final class SHA3 {
    ...
    static var shared = SHA3()
    
    var messageBytes: [UInt8] = []
    var messageBlocks: [UInt64] = []
    ...
    private func additing() {
        
    }
    
    private func messageParse() {
    ...
    private func Delta0(_ x: UInt64) -> UInt64 {
        return ROTR(x, 1) ^ ROTR(x, 8) ^ SHR(x, 7)
    }
    
    private func Delta1(_ x: UInt64) -> UInt64 {
        return ROTR(x, 19) ^ ROTR(x, 61) ^ SHR(x, 6)
    }
    
    private func ROTL(_ x: UInt64, _ n: UInt) -> UInt64 {
        return (x << n) | (x >> (64-n))
    }
    
    private func ROTR(_ x: UInt64, _ n: UInt) -> UInt64 {
        return (x >> n) | (x << (64-n))
    }
    
    private func SHR(_ x: UInt64, _ n: UInt) -> UInt64 {
        return x >> n
    }
